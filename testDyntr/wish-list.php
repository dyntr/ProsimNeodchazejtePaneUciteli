...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
    ...
    if (empty($errors)) {
        $sql = "INSERT INTO Animal (name, type, image_path, owned, user_id, is_public) VALUES (?, ?, ?, 0, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
...
// Search Operation
$search = $_GET['search'] ?? '';
$sql = "SELECT id, name, type, image_path FROM Animal WHERE user_id = ? AND owned = 0 AND (name LIKE ? OR type LIKE ?)";
$stmt = mysqli_prepare($conn, $sql);
$like_search = '%' . $search . '%';
